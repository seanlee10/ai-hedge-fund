...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    if len(historical_earnings) >= 3:
        ...
        if oldest_earnings > 0 and latest_earnings > 0:
            # Both positive - can calculate CAGR normally
        ...
        elif oldest_earnings > 0 and latest_earnings <= 0:
            # Earnings went from positive to negative - use conservative negative growth
            conservative_growth = -0.05
        else:
            conservative_growth = 0.03  # Default 3% if negative or zero base
    else:
